...
class ServerObject
{
    ...
    {
        //идея: метод запускается в завершении каждого кода. Проверяется количество кард на руках каждого игрока и при необходимости ему выдаются карты
        for (int i = 0; i < clients.Count; i++)
        {
            if (clients[i].de.cards.Length < 6)
            {
                HttpClient htclient = new HttpClient();
                int cardneed = 6 - clients[i].de.cards.Length;
                string responseBody = await htclient.GetStringAsync($"https://deckofcardsapi.com/api/deck/{clients[i].de.deck_id}/draw/?count={cardneed}");
                ...
                // Send the data to the client asynchronously
...
class ClientObject
{
    ...
    protected internal HttpClient htclient { get; } = new HttpClient(); // БЫЛ ПРИВАТНЫМ, СТАЛ PROTECTED ITERNAL
    protected internal TcpClient client { get; } // // БЫЛ ПРИВАТНЫМ, СТАЛ PROTECTED ITERNAL
    private ServerObject server;
    private string? userName;
    private static readonly object fileLock = new object();
    protected internal deck? de { get; set;  } = null; // БЫЛ ПРИВАТНЫМ, СТАЛ PROTECTED ITERNAL
    ...
    async Task createnewdeck() // вызывыется в начале игры для каждого игрока по одному разу.
    {
        ...
        try
        {
            ...
            //string responseBody = await response.Content.ReadAsStringAsync();
            // Above three lines can be replaced with new helper method below
            string responseBody = await htclient.GetStringAsync("https://deckofcardsapi.com/api/deck/new/shuffle/?deck_count=6"); // цифра после count отвечает за количество карт, которые возьмет и запишет сервер
            //Console.WriteLine(responseBody); // вывод всего запроса json
            de = JsonSerializer.Deserialize<deck>(responseBody); // запись в массивы данных из запроса
